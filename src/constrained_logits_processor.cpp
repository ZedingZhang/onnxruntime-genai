...
std::shared_ptr<const GuidanceTokenizerAsset> CreateTokenizerAsset(
  ...
  auto tokenize_fn = (LlgTokenizeFn) + [](const void* user_data, const uint8_t* bytes,
                                          ...
                                          size_t output_tokens_len) -> unsigned long {
...
std::shared_ptr<const GuidanceGrammarAsset> GetGrammarAsset(
  ...
  std::shared_future<std::shared_ptr<const GuidanceGrammarAsset>> future;
  {
    ...
    if (existing != cache->grammars.end()) {
      ...
      future = promise->get_future().share();
      cache->grammars.emplace(key, GuidanceCacheState::GrammarEntry{future, cache->lru.end(), false});
  ...
  if (promise) {
    try {
      ...
      auto asset = CompileGrammarAsset(params, std::move(tokenizer_asset));
      ...
      while (cache->lru.size() > kMaxCachedGrammars ||
             cache->cached_key_bytes > kMaxCachedGrammarKeyBytes) {
...
// remain model-owned while every mutable constraint is cloned per request.
